...
/*
    serial_out("Pt input := ", (uint8_t*)&enc, NULL, 24);
*/
    //asmSB((uint8_t*)TempState);
...
void Decrypt(StateUint Ciphertext, StateUint Key, uint8_t *dec) {
    ...
    serial_out("Ct input := ", (uint8_t*)&TempState, NULL, 24);
    ...
    //serial_out("TempState 0 := ", (uint8_t*)&TempState, NULL, 24);
    ...
    for(int r = NR - 2; r >= 0; r--) {
        ...
        //serial_out("TempState i := ", (uint8_t*)&TempState, NULL, 24);
    }
    //serial_out("TempState f := ", (uint8_t*)&TempState, NULL, 24);
    uint8_t pr[24] = {0};
    print_state2(TempState, pr);
    serial_out2("Decryption pr := ", (uint8_t*)&pr);
